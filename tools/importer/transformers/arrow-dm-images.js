...
/**
 * Transformer: arrow.com Dynamic Media / Scene7 images.
 *
 * arrow.com serves imagery from Scene7 IS/Image (media.arrow.com/is/image/...)
 * and DM Open API. Confirmed in migration-work/metadata.json `.images.mapping`
 * (20+ media.arrow.com/is/image/ keys). Converts DM/S7 <img> tags into anchors
 * so the URLs round-trip through markdown intact; the companion auto-block in
 ...
 * <img> references into block cells (cards/carousel/columns). Rewriting to
 * anchors in beforeTransform would leave parsers with empty image cells.
 *
 * Helpers below are the transformer-side subset copied byte-identically from
...
export default function transform(hookName, element, payload) {
  ...
  element.querySelectorAll('img').forEach((img) => {
    ...
    // anchors; when alt is empty we substitute EMPTY_ALT_SENTINEL so authors
    // see a visible cell, and the auto-block translates it back to alt="".
